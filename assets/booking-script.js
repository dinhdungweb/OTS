...
function isTimeExpired(dateString, timeString) {
  ...
  bookingForm.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      if (result.success) {
        showModal(true, 'BẠN ĐÃ ĐẶT LỊCH THÀNH CÔNG', 'OfTheSun đã nhận được thông tin đăng ký từ bạn – Đừng quên kiểm tra email để xem lại chi tiết cuộc hẹn!');
        
        // Gửi email xác nhận
        try {
          if (typeof emailjs !== 'undefined') {
            await emailjs.send('service_1myqfes', 'booking_confirmation', {
